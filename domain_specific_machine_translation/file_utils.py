...
def read_txt(path: str) -> list[str]:
    """
    Read txt file and returns it as list of strings, one string per line.
...
def write_txt(lines: list[str], path: str):
    """
    Write a list of strings and write it to a txt file, one string per line.
...
def write_jsonl(dict_: dict, path: str):
    """
    Write a dicts values to JSONL format file, ignoring the keys.
